...
function AddEmployee() {
  ...
  useEffect(() => {
    // Reset services when department changes or is empty
    ...
    const fetchServices = async () => {
      ...
      try {
        console.log("Fetching services for department:", formData.department);
        const res = await axios.get(
          `${API_URL}/api/getServicebyDepartment/${formData.department}`
        );
        ...
        console.error("Error fetching services:", error);
        alert("Failed to fetch services for this department");
        setServices([]);
      } finally {
  ...
  const handleChange = (e) => {
    ...
    // If department changes, reset service selection
    if (name === 'department') {
      setFormData((prevData) => ({
        ...prevData,
        department: value,
        ...
        <div className="card mb-4">
          ...
          <div className="card-body row g-3">
            ...
            <div className="col-md-6">
              <label>Official Email</label>
              <input type="email" className="form-control" name="official_email"
                value={formData.official_email} onChange={handleChange} required />
            </div>
            <div className="col-md-6">
              <label>Password</label>
              <input type="password" className="form-control" name="password"
          ...
          <div className="card-body row g-3">
            ...
            <div className="col-md-6">
              ...
              <select 
                ...
                onChange={handleChange}
                disabled={!formData.department || loadingServices}
                required
              >
                <option value="">
                  {loadingServices 
              ...
              {formData.department && services.length === 0 && !loadingServices && (
                <small className="text-danger">No services available for this department</small>
              )}
            </div>
            <div className="col-md-6">
              <label>Interview Date</label>
            ...
            {(formData.userType === 'trainee' || formData.userType === 'intern') && (
              <div className="col-md-6">
                ...
                <select 
                  ...
                  value={formData.traineeDuration} 
                  onChange={handleChange}
                >
                  <option value="">Select Duration</option>
                  <option value="3">3 Months</option>
