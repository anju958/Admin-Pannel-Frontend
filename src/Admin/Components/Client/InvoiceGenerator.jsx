...
// // Get readable project name robustly
...
//   // Extra robust final fallback for showing project names for created invoice
//   const getFinalProjectName = (p) => {
//     // Try all direct fields (backend should always set projectName)
//     if (p.projectName || p.name || p.project || p.title)
//       return p.projectName || p.name || p.project || p.title;
//     // Final fallback: look up in original client list if projectId matches
//     if (p.projectId && projects.length) {
//       const orig =
//         projects.find(
//           (pr) => `${pr._id}` === `${p.projectId}`
//         );
//       return getProjectDisplayName(orig);
//     }
...
//                       className="list-group-item d-flex justify-content-between align-items-center"
//                     >
//                       <span>{getFinalProjectName(p)}</span>
//                       <input
//                         type="number"
