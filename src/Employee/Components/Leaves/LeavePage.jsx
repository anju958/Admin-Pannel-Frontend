...
function LeavePage() {
  ...
  const [cards, setCards] = useState([
    { id: 1, title: "Leaves Taken (This Month)", number: 0 },
  ]);
  const [leave, setLeave] = useState({
    employeeId: user?.employeeId || user?._id || "",
    employeeName: user?.ename || "",
    leave_type: "",
    from_date: "",
    to_date: "",
    reason: "",
  ...
  const fetchLeaves = async () => {
    ...
    try {
      const leaveRes = await axios.get(
        `${API_URL}/api/getAllLeaves/${employee._id || employee.employeeId}`
      );
      const leaves = leaveRes.data || [];
      setLeaveHistory(leaves);
      ...
      const now = new Date();
      const currentMonthLeaves = leaves.filter((lv) => {
        const d = new Date(lv.from_date || lv.startDate);
        return (
          d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear()
        );
      });
      setCards([
        { id: 1, title: "Leaves Taken (This Month)", number: currentMonthLeaves.length },
      ]);
    } catch (err) {
      console.error("Error fetching leaves:", err);
    }
  ...
  return (
    ...
    >
      {/* Top Cards */}
      <div className="container-fluid py-3" style={{ flexShrink: 0 }}>
        <Box
          sx={{
            display: "grid",
      ...
      >
        ...
        <div className="container-fluid bg-white p-4 rounded shadow-sm">
          <h2 className="text-center mb-3">Apply for Leave</h2>
        ...
        <div
          ...
          style={{ backgroundColor: "#d4edda", borderRadius: "5px" }}
        >
          <div className="d-flex justify-content-end mb-2">
            <label className="me-2 fw-semibold">Select Month:</label>
            <input
